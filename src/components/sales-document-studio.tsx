...
type Deck = { id: string; title: string; purpose: DeckPurpose; language: MarketingLanguage; audience: string | null; version: string | null; notes: string | null; tags: string[]; format: string; status: string; marketing_deck_files?: DeckFile[] };
...
export function SalesDocumentStudio() {
  ...
  const files = useMemo(() => (decksQ.data ?? []).flatMap((deck) => (deck.marketing_deck_files ?? []).map((file) => ({ deck, file }))), [decksQ.data]);
