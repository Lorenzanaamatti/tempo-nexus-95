...
type Composition = { id: string; title: string; purpose: DeckPurpose; language: MarketingLanguage; status: string; notes: string | null; created_at: string; sales_document_outputs?: Array<{ id: string; storage_path: string; file_name: string }> };
...
export function SalesDocumentStudio() {
  ...
  const compositionsQ = useQuery({ queryKey: ["sales-document-compositions"], queryFn: async () => {
    const { data, error } = await (supabase as any).from("sales_document_compositions").select("*, sales_document_outputs(id, storage_path, file_name)").order("created_at", { ascending: false });
    if (error) throw error;
...
function OutputDownload({output}:{output:{storage_path:string;file_name:string}}) { const [url,setUrl]=useState<string|null>(null); async function download(){const signed=await signMarketingAsset(output.storage_path);if(!signed)return toast.error("No se pudo preparar la descarga");setUrl(signed);window.open(signed,"_blank","noopener,noreferrer");} return <Button size="sm" variant="outline" onClick={download}><Download className="mr-1 h-4 w-4"/>{url?"Abrir PDF":output.file_name}</Button>; }
