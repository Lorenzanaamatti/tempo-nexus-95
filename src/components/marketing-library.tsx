...
/**
 * Generic marketing asset library: one section (identidad, ventas, comunicacion,
 * templates) split into category tabs. All data lives in brand_assets /
 * brand_asset_files, filtered by section + category.
 */
export function MarketingLibrary({
  ...
  const assetsQ = useQuery({
    queryKey: ["marketing-library", section],
    queryFn: async () => {
  ...
  const filesQ = useQuery({
    queryKey: ["marketing-library-files", section, category],
    enabled: view !== "collections",
    queryFn: async () => {
      const ids = assets.map((a) => a.id);
      if (!ids.length) return [] as (AssetFile & { asset_title?: string | null })[];
      const { data, error } = await (supabase as any)
        .from("brand_asset_files")
        .select("*, brand_assets!inner(title)")
        .in("asset_id", ids)
        .order("created_at", { ascending: false });
      if (error) throw error;
      return ((data ?? []) as any[]).map((r) => ({ ...r, asset_title: r.brand_assets?.title ?? null }));
    },
  });

  const filteredFiles = (filesQ.data ?? []).filter((f) => {
    if (!query.trim()) return true;
    const q = query.toLowerCase();
    return (f.filename ?? "").toLowerCase().includes(q) || (f.asset_title ?? "").toLowerCase().includes(q);
  });

  async function addAsset() {
    const label = categories.find((c) => c.key === category)?.label ?? "recurso";
    const t = prompt(`Nombre del recurso en ${label}:`);
    if (!t?.trim()) return;
    const max = (assetsQ.data ?? []).reduce((m, a) => Math.max(m, a.position), -1);
    const { error } = await (supabase as any)
      .from("brand_assets")
      .insert({ title: t.trim(), position: max + 1, section, category });
      ...
      <section>
        ...
        {view !== "collections" && (
          <div className="mb-3 flex items-center justify-between gap-2">
            <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar archivo o colección…" className="h-8 max-w-sm text-sm" />
        ...
        {view === "collections" ? (
          !assets.length ? (
            <p className="rounded-sm border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
              Aún no hay recursos en esta categoría. Crea uno y sube tantos archivos como necesites.
            </p>
          ) : (
            <ul className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {assets.map((a) => <AssetCard key={a.id} item={a} section={section} />)}
            </ul>
          )
        ) : view === "grid" ? (
          <FilesGrid files={filteredFiles} onChanged={() => qc.invalidateQueries({ queryKey: ["marketing-library-files", section, category] })} />
        ) : (
          <FilesList files={filteredFiles} onChanged={() => qc.invalidateQueries({ queryKey: ["marketing-library-files", section, category] })} />
        )}
      </section>
    </div>
  );
...
function ViewBtn({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn("inline-flex items-center gap-1 px-2.5 py-1 text-xs transition-colors", active ? "bg-primary text-primary-foreground" : "bg-transparent hover:bg-muted")}
    >
      {icon}
      {label}
    </button>
  ...
  if (!definitelyNotImage && url && !failed) {
    return (
      <div className="h-full w-full bg-[repeating-conic-gradient(hsl(var(--muted))_0%_25%,transparent_0%_50%)] bg-[length:16px_16px]">
        <img src={url} alt={file.filename ?? ""} className="h-full w-full object-contain" loading="lazy" onError={() => setFailed(true)} />
      </div>
    );
  }
  return <div className="flex h-full w-full items-center justify-center bg-muted text-muted-foreground"><FileIcon className="h-6 w-6" /></div>;
}

function AssetCard({ item, section }: { item: Asset; section: string }) {
  const qc = useQueryClient();
  ...
  const filesQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("brand_asset_files").select("*").eq("asset_id", item.id).order("position").order("created_at");
      if (error) throw error;
  ...
  async function save() {
    const { error } = await (supabase as any).from("brand_assets")
      .update({ title: a.title, kind: a.kind, external_url: a.external_url, notes: a.notes }).eq("id", a.id);
    if (error) return toast.error(error.message);
  ...
  async function remove() {
    ...
    for (const f of filesQ.data ?? []) await deleteMarketingAsset(f.storage_path).catch(() => {});
  ...
  return (
    <li className="glass-panel rounded-sm border border-border p-4">
      ...
      <Input value={a.kind ?? ""} onChange={(e) => setA({ ...a, kind: e.target.value || null })} placeholder="Tipo / etiqueta" className="mb-2 text-sm" />
